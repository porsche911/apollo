...
public class MybatisProxy {
    public static Object getInstance(Object target) throws MalformedURLException, ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        ...
        String constructorContent = tab+"public $ProxyMybatis ("+infName+" target){"+line
        ...
        for (Method method : methods) {
            ...
            if (returnTypeName.equals("void")){
                methodContent+=tab+tab+"target."+methodName+"("+paramsContents+");"+line
                        +tab+"}"+line;
            }else {
                methodContent+=tab+tab+"return target."+methodName+"("+paramsContents+");"+line
                        +tab+"}"+line;
            }

            /*methodContent+=tab+"public "+returnTypeName+" "+methodName+"("+argsContent+") {"+line
                    +tab+tab+invocation+"."+invokeMethodsName+"();"+line
                    +tab+tab+"target."+methodName+"("+paramsContents+");"+line
                    +tab+"}"+line;*/
        }
        ...
        Object o = constructor.newInstance(target);//通过有参构造器生成对象
        return o;
    }
}
